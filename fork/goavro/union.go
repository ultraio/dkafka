...
func (cr *codecInfo) nullable() bool {
	if len(cr.allowedTypes) == 2 {
		_, found := cr.indexFromName["null"]
		return found
	}
	return false
}
...
func binaryFromNative(cr *codecInfo) func(buf []byte, datum interface{}) ([]byte, error) {
	return func(buf []byte, datum interface{}) ([]byte, error) {
		...
		case map[string]interface{}:
			if len(v) != 1 {
				err = fmt.Errorf("cannot encode binary union: non-nil Union values ought to be specified with Go map[string]interface{}, with single key equal to type name, and value equal to datum value: %v; received: %T", cr.allowedTypes, datum)
			} else {
		...
		if cr.nullable() {
			...
			return doBinaryFromNative(cr, index, buf, datum)
		}
		if err != nil {
			return nil, err
		}
...
func doBinaryFromNative(cr *codecInfo, index int, buf []byte, value interface{}) ([]byte, error) {
	c := cr.codecFromIndex[index]
