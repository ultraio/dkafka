...
func intBinaryFromNative(buf []byte, datum interface{}) ([]byte, error) {
	...
	case uint32:
		if value = int32(v); value < 0 {
			return nil, fmt.Errorf("cannot encode binary long: provided Go uint32 would lose precision: %d", v)
		}
	case int64:
	...
	default:
		...
		case reflect.Int, reflect.Int8, reflect.Int16,
			...
			if value = int32(valueOf.Int()); valueOf.Int() != int64(value) {
				return nil, fmt.Errorf("cannot encode binary int: provided Go %s would lose precision: %d", reflect.TypeOf(v), v)
			}
		case reflect.Uint, reflect.Uint8, reflect.Uint16,
			reflect.Uint32, reflect.Uint64:
			if value = int32(valueOf.Uint()); value < 0 {
				return nil, fmt.Errorf("cannot encode binary int: provided Go %s would lose precision: %d", reflect.TypeOf(v), v)
			}
		case reflect.Float32, reflect.Float64:
			if value = int32(valueOf.Float()); valueOf.Float() != float64(value) {
				return nil, fmt.Errorf("cannot encode binary int: provided Go %s would lose precision: %d", reflect.TypeOf(v), v)
			}
		default:
...
func longBinaryFromNative(buf []byte, datum interface{}) ([]byte, error) {
	...
	default:
		...
		case reflect.Uint, reflect.Uint32, reflect.Uint64:
			if value = int64(valueOf.Uint()); value < 0 {
				return nil, fmt.Errorf("cannot encode binary long: provided Go %s would lose precision: %d", reflect.TypeOf(v), v)
			}
		case reflect.Float32, reflect.Float64:
			if value = int64(valueOf.Float()); valueOf.Float() != float64(value) {
				return nil, fmt.Errorf("cannot encode binary long: provided Go %s would lose precision: %d", reflect.TypeOf(v), v)
			}
		default:
			return nil, fmt.Errorf("cannot encode binary long: expected: Go numeric; received: %T", datum)
		}
	}
